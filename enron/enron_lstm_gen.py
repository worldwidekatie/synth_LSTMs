...
def lstm_gen_spam(label, num, noise):
    data = "spam"
    # Read in the data
    df = pd.read_csv(f"enron/enron_data/{data}_seed_05.csv")
    text_ = " "
    ...
    # Save Model
    model.save(f'enron_{data}_lstm')
    ...
    var.to_csv(f"enron_{data}_lstm_noise_{noise_[2:]}.csv", index=False)
...
def concatonate(noise):
    noise_ = str(noise)
    df1 = pd.read_csv(f"enron_ham_lstm_noise_{noise_[2:]}.csv")
    df2 = pd.read_csv(f"enron_spam_lstm_noise_{noise_[2:]}.csv")

    df = pd.concat([df1, df2])
    df.to_csv(f"enron_lstm_train_noise_{noise_[2:]}.csv", index=False)
