...
# For pos Model
def lstm_gen_pos(label, num, noise):
    data = "pos"
    # Read in the data
    df = pd.read_csv(f"imbd/imbd_data/{data}_seed_05.csv")
    text_ = " "
    ...
    def gen_data(number):
        ...
        for i in range(0, number):
            ...
            for i in senten:
                sentence += i
            for i in range(1, random.randint(41, 190)):
    ...
    var.to_csv(f"imbd_{data}_lstm_noise_{noise_[2:]}.csv", index=False)
...
# For neg Model 
def lstm_gen_neg(label, num, noise):
    # Read in the data
    data = "neg"
   ...
   # Noise
    ...
    def gen_data(number):
        ...
        for i in range(0, number):
            ...
            for i in senten:
                sentence += i
            for i in range(1, random.randint(82, 179)):
                x_pred = np.zeros((1, maxlen, len(chars)))
                for t, char in enumerate(senten):
                    x_pred[0, t, char_int[char]] = 1
                preds = model.predict(x_pred, verbose=0)[0]
...
def concatonate(noise):
    noise_ = str(noise)
    df1 = pd.read_csv(f"imbd_neg_lstm_noise_{noise_[2:]}.csv")
    df2 = pd.read_csv(f"imbd_pos_lstm_noise_{noise_[2:]}.csv")

    df = pd.concat([df1, df2])
    df.to_csv(f"imbd_lstm_train_noise_{noise_[2:]}.csv", index=False)


def run(noise):
    lstm_gen_pos(1, 360, noise)
    lstm_gen_neg(0, 320, noise)
    concatonate(noise)
